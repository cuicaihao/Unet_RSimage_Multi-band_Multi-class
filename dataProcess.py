...
def trainGenerator(batch_size, train_image_path, train_label_path, classNum, colorDict_GRAY, resize_shape=None):
    ...
    #  无限生成数据
    while(True):
        img_generator = np.zeros(
            (batch_size, img.shape[0], img.shape[1], img.shape[2]), np.uint8)
        label_generator = np.zeros(
            (batch_size, img.shape[0], img.shape[1]), np.uint8)
        if(resize_shape != None):
            img_generator = np.zeros(
                (batch_size, resize_shape[0], resize_shape[1], resize_shape[2]), np.uint8)
            label_generator = np.zeros(
                (batch_size, resize_shape[0], resize_shape[1]), np.uint8)
        #  随机生成一个batch的起点
        ...
        for j in range(batch_size):
            ...
            label = readTif(train_label_path + "/" +
                            labelList[rand + j]).astype(np.uint8)
            #  若为彩色，转为灰度
        ...
        img_generator, label_generator = dataPreprocess(
            img_generator, label_generator, classNum, colorDict_GRAY)
...
def saveResult(test_image_path, test_predict_path, model_predict, color_dict, output_size):
    ...
    for i, img in enumerate(model_predict):
        ...
        #  修改差值方式为最邻近差值
        img_out = cv2.resize(
            img_out, (output_size[0], output_size[1]), interpolation=cv2.INTER_NEAREST)
        #  保存为无损压缩png
        cv2.imwrite(test_predict_path + "/" +
